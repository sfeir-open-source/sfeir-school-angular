...
import { ReactiveForm } from '@sfeir/ui-solution/reactive-form';
...
describe('DialogPerson', () => {
  ...
  describe('Template', () => {
    it('should call closeDialog with the form value when the submitForm event is triggered', async () => {
      ...
      const reactiveFormComponent = debugElement.query(By.directive(ReactiveForm));
      reactiveFormComponent.triggerEventHandler('submitForm', PERSON_BODY);
    ...
    it('should call closeDialog with no value when the cancelForm event is triggered', async () => {
      const spy = vi.spyOn(component, 'closeDialog');
      const reactiveFormComponent = debugElement.query(By.directive(ReactiveForm));
      reactiveFormComponent.triggerEventHandler('cancelForm');
      await fixture.whenStable();
