...
describe('PeopleService', () => {
  ...
  beforeEach(async () => {
    TestBed.configureTestingModule({
      providers: [provideHttpClient(), provideHttpClientTesting(), PeopleService, { provide: AppStore, useValue: APP_STORE }],
    });
  ...
  describe('#getPeople', () => {
    ...
    test('should return the correct data', ({ expect }) => {
      service.getPeople().subscribe(data => expect(data).toEqual(PEOPLE));
      const req = controller.expectOne(`${environment.peopleEndpoint}/peoples`);
      req.flush(PEOPLE);
    });
    test('should set the people to the store', ({ expect }) => {
      service.getPeople().subscribe(() => {
        expect(APP_STORE.setPeople).toHaveBeenCalledWith(PEOPLE);
      });
      const req = controller.expectOne(`${environment.peopleEndpoint}/peoples`);
      req.flush(PEOPLE);
    });
  ...
  describe('#deletePeople', () => {
    ...
    test('should return the correct data', ({ expect }) => {
      service.deletePeople(PEOPLE[0].id).subscribe(data => expect(data).toEqual([PEOPLE.at(1)]));
      const req = controller.expectOne(`${environment.peopleEndpoint}/peoples/${PEOPLE[0].id}`);
      req.flush([PEOPLE.at(1)]);
    });
    test('should set the people to the store', ({ expect }) => {
      service.deletePeople(PEOPLE[0].id).subscribe(() => expect(APP_STORE.setPeople).toHaveBeenCalledWith([PEOPLE.at(1)]));
      const req = controller.expectOne(`${environment.peopleEndpoint}/peoples/${PEOPLE[0].id}`);
      req.flush([PEOPLE.at(1)]);
    });
    test('should throw correctly the error', ({ expect }) => {
  ...
  describe('#getPersonDetails', () => {
    ...
    test('should return the person details', ({ expect }) => {
      service.getPersonDetails('123').subscribe(personDetails => expect(personDetails).toEqual(PEOPLE[0]));
