...
const DISPATCHED_ACTION = vi.fn(() => void 0);

vi.mock('@ngxs/store', async () => {
  const library = await vi.importActual<any>('@ngxs/store');
  return {
    ...library,
    select: vi.fn(() => signal(PEOPLE)),
    dispatch: vi.fn(() => DISPATCHED_ACTION),
  };
});
...
describe('PeopleComponent', () => {
  ...
  describe('#card-view-mode', () => {
    ...
    test('should call the delete method', () => {
      vi.spyOn(PEOPLE_SERVICE, 'deletePeople').mockReturnValue(of([PEOPLE[1]]));
      const sfeirCards = container.querySelectorAll('sfeir-card');
      expect(sfeirCards.length).toEqual(2);
