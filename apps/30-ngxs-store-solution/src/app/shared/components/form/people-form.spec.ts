...
describe('Form', () => {
  ...
  test('should the email controls be required', () => {
    expect(form.controls.email.errors).toEqual({ required: true, sfeirEmail: true });
    form.controls.email.patchValue('a');
    expect(form.controls.email.errors).toEqual({ sfeirEmail: true });
    form.controls.email.patchValue('frizzarin.n@sfeir.com');
    expect(form.controls.email.errors).toEqual(null);
  });
