...
@Component({
  imports: [SfeirCustomInput, ReactiveFormsModule],
  template: ` <sfeir-custom-input inputPlaceholder="test" inputType="text" [formControl]="control">Loose focus </sfeir-custom-input>`,
})
class HostComponent {
...
describe('SfeirCustomInput', () => {
  ...
  test('should not display the message lose focus', () => {
    expect(container.textContent).not.toContain('Loose focus');
  });
  ...
  test('should display the message lose focus', () => {
    ...
    expect(container.textContent).toContain('Loose focus');
