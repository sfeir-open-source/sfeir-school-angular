...
function getSlideFilesFromSlidesJs(rootDir) {
  return __async(this, null, function* () {
    ...
    if (typeof slideJs.formation !== 'function') {
      throw new Error('scripts/slides.js should have an exported function formation');
    }
...
function importSlidesJs(rootDir) {
  try {
    return (
      ...
      encodeURIComponent(
        fs2
          .readFileSync(path4.resolve(docsPath(rootDir), 'scripts/slides.js'), 'utf-8')
          .split('\n')
...
function getAllCssContent(rootDir, extraCssFiles) {
  return THEME_CSS_FILES.concat(extraCssFiles)
    .map(cssFilePath => getCssFile(rootDir, cssFilePath))
    .concat(OTHER_KNOWN_CSS_CLASSES)
    .join('\n\n\n');
}
...
var init_css_utils = __esm({
  'cli/utils/css.utils.ts'() {
    'use strict';
    init_path_utils();
    THEME_CSS_FILES = ['web_modules/sfeir-school-theme/dist/sfeir-school-theme.css', 'css/slides.css'];
    OTHER_KNOWN_CSS_CLASSES = ['.list-fragment', '.transition-bg-sfeir-1', '.transition-bg-sfeir-2', '.transition-bg-sfeir-3'];
  },
});
...
function getLabPackageJson(rootDir, lab) {
  try {
    return JSON.parse(fs4.readFileSync(labPackageJsonPath(rootDir, lab), 'utf-8'));
  } catch (e) {
    return null;
  }
}
...
function checkDocs(rootDir, config) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      check('S_010', { msg: `slides.js should export "formation" function`, continueCheck: false }, () => false);
      return;
    }
...
function checkSlideFilePathInSlideJs(rootDir, slideFilesFromSlidesJs) {
  for (const slideFile of slideFilesFromSlidesJs) {
    ...
    check('S_002', `slides.js entry "${slideFile == null ? void 0 : slideFile.path}" does not match an existing file`, () =>
...
function checkSlideFileInFs(rootDir, slideEntriesFromSlidesJs, config) {
  ...
  for (const slideFile of slideFilesFromFs) {
    check('S_003', `"${slideFile}" should be used`, () => slideFilesFromSlidesJs.includes(slideFile));
    for (const imagePath of getImagesPathFromSlides(rootDir, readSlideFile(rootDir, slideFile))) {
      check('S_007', `"${imagePath}" in "${slideFile}" should be an existing images`, () => isImageFileExists(imagePath));
    }
    for (const cssClass of getCssClassUsedInSlide(readSlideFile(rootDir, slideFile))) {
      check('S_009', `"${cssClass}" in "${slideFile}" is not a known css class`, () => cssContent.includes(cssClass));
    }
  }
}
function checkLabSlideFile(rootDir, slideFilesFromSlidesJs, config) {
  ...
  for (const slideFile of labSlides) {
    ...
    const hasCommandRow = check('S_004', `"${slideFile == null ? void 0 : slideFile.path}" should contains the command to run the exercise`, () => {
      return isDefined(commandRow) && commandRow.length > 0;
    });
    if (hasCommandRow && isDefinedAndNotEmpty(config.stepCommandPrefix)) {
      check('S_005', `"${slideFile == null ? void 0 : slideFile.path}" should contains the valid command to run the exercise`, () => {
        const commandTarget = getLabCommandTarget(commandRow, config);
        return isLabCommandExists(rootDir, commandTarget);
      });
    }
    check('S_006', `"${slideFile == null ? void 0 : slideFile.path}" should use lab slide format`, () => {
      const slideRows = labSlideContent.split('\n').map(row => row.trim());
      return slideRows.includes('<!-- .slide: class="exercice" -->') && slideRows.includes('## Lab');
    });
  }
}
function checkLabCommand(rootDir, labSlides, config) {
  const allLabSlides = labSlides.map(slide => readSlideFile(rootDir, slide.path));
...
function checkLabDirectories(rootDir, config) {
  ...
  for (const labDir of labDirectories) {
    ...
    if ((stepsPackageJson == null ? void 0 : stepsPackageJson.kind) === 'package.json') {
      check('L_003', `Lab "${labDir}" should have corresponding script`, () => labScripts.includes(labDir));
      check('L_004', `Lab "${labDir}" should have a package.json with corresponding name`, () => {
        var _a;
...
function checkLabsAndSolutions(rootDir, config) {
  ...
  for (const lab of labs) {
    ...
    const hasReadme = check('L_005', `Lab "${lab}" should have a README.md`, () => isDefined(readme) && readme.length > 0);
    if (hasReadme) {
      check('L_006', `Lab "${lab}"'s README.md should contains the correct title`, () => {
        var _a;
        return (_a = readme == null ? void 0 : readme.includes(`# ${lab} instructions`)) != null ? _a : false;
      });
      check('L_007', `Lab "${lab}"'s README.md should contains the correct command`, () => {
        var _a;
        return (_a = readme == null ? void 0 : readme.includes(`${config.stepCommandPrefix}${lab}`)) != null ? _a : false;
      });
    }
    check('L_008', `Lab "${lab}" should have a solution`, () => labSolutions.includes(lab + '-solution'));
  }
  for (const labSolution of labSolutions) {
    const matchingLabName = labSolution.replace('-solution', '');
    const hasMatchingLab = check('L_009', `Solution lab "${labSolution}" should match to a lab`, () => labs.includes(matchingLabName));
    if (hasMatchingLab) {
      const labReadme = getLabReadme(rootDir, matchingLabName);
      const labSolutionReadme = getLabReadme(rootDir, labSolution);
      check('L_010', `Lab and solution of "${matchingLabName}" should have same README.md`, () => labReadme === labSolutionReadme);
    }
  }
}
...
function explainCommand({ ruleCode }) {
  ...
  docSection = docSection.slice(
    0,
    docSection.slice(1).findIndex(row => row.startsWith('##### ')),
  );
...
function initConfigCommand({ rootDir }) {
  ...
  fs6.writeFileSync(configPath, JSON.stringify(config_template_default, void 0, 2));
}
  ...
  'package.json'() {
    package_default = {
      ...
      scripts: {
        test: 'vitest',
        serve:
          'live-server --port=4242 --open="./demo/index.html" --mount="/assets:./demo/assets" --mount="/web_modules:./demo/web_modules" --watch="./demo/web_modules/sfeir-school-theme,./demo/markdown/"',
        ...
        'build:scripts':
          'tsc -p tsconfig.scripts.json && chmod +x dist/sfeir-school-theme-cleanup.js dist/sfeir-school-theme-migration.js && mv dist/sfeir-school-theme-cleanup.js dist/sfeir-school-theme-cleanup.mjs && mv dist/sfeir-school-theme-migration.js dist/sfeir-school-theme-migration.mjs',
