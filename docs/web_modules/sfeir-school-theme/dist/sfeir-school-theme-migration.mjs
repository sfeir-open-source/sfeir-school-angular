  ...
  MARKDOWN: [
    // 1. Icônes avec style (plus spécifique en premier)
    {
      ...
      to: (_match, _markdownImage, modifiers, iconName, styleContent) => {
        ...
        const newStyleContent = styleContent.replace(/--icon-size/g, '--tc-icon-size').replace(/--icon-color/g, '--tc-icon-color');
...
function migrateMultiColumnSlides(content) {
  ...
  const processedSections = sections.map(section => {
    ...
    if (notes) {
      contentWithoutNotes = slideContent.replace(/(Notes:\s*[\s\S]*?)(?=##--##|##==##|$)/, '').trim();
    }
    ...
    const processedColumns = columns
      .map((col, colIndex) => {
        ...
        if (slideMatch) {
          ...
          const contentAfterSlideTag = col.replace(/<!--\s*\.slide:\s*[^>]*?\s*-->/, '').trim();
...
function findFiles(dir, filter) {
  ...
  list.forEach(function (file) {
    ...
    if (path.basename(fullPath) === 'web_modules' || path.basename(fullPath) === 'node_modules') {
